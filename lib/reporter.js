        ...
        for (let cid of Object.keys(stats.runners)) {
            ...
            for (let specId of Object.keys(runner.specs)) {
                ...
                for (let suiteName of Object.keys(spec.suites)) {
                    ...
                    let skippableTests = ['"before" hook', '"before each" hook', '"after each" hook']
                    let errors = []
                    ...
                    for (let testName of Object.keys(suite.tests)) {
                        ...
                        if (test.state === 'fail') {
                            ...
                            if (testName === '"before all" hook') {
                                skippableTests.push('"after all" hook')
                            }
                        }
                        if (errors.length === 0) {
                            skippableTests.push('"before all" hook')
                            skippableTests.push('"after all" hook')
                        }
                        if (skippableTests.indexOf(testName) >= 0) {
                            continue
                        }
